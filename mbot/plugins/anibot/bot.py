...
from config import BOT_NAME, TRIGGERS as trg, HELP_DICT
from mbot.ani_utils.db import get_collection
from mbot.ani_utils.helper import AUTH_USERS, clog, check_user, control_user
from mbot.ani_utils.data_parser import get_additional_info
...
async def en_dis__able_cmd(client: Client, message: Message):
    ...
    if user in SUDO_USERS or (await anibot.get_chat_member(gid, user)).status!='member':
        if len(cmd)==1:
...
@Client.on_message(filters.command(['anstar', f'start{BOT_NAME}'], prefixes=trg))
@control_user
async def start_b(client: Client, message: Message):
    gid = message.chat.id
    user = message.from_user.id
    find_gc = await DC.find_one({'_id': gid})
    ...
    if gid==user:
        ...
        if not (user in SUDO_USERS) and not (await USERS.find_one({"id": user})):
...
@Client.on_message(filters.command(['anhelp', f'help{BOT_NAME}'], prefixes=trg))
@control_user
async def helphb_(client: Client, message: Message):
    gid = message.chat.id
    find_gc = await DC.find_one({'_id': gid})
...
@Client.on_message(filters.user(SUDO_USERS) & filters.command(['stats', f'stats{BOT_NAME}'], prefixes=trg))
@control_user
...
async def feed_(client: Client, message: Message):
    owner = await client.get_users(SUDO_USERS[0])
...
@Client.on_message(filters.command(['eval', f'eval{BOT_NAME}'], prefixes=trg) & filters.user(SUDO_USERS))
@control_user
